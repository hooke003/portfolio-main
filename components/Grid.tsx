...
type GridItem = Omit<BentoGridItemProps, "size"> & {
  size?: BentoGridItemProps["size"];
};
...
const Grid: React.FC = () => {
  ...
  const validGridItems = useMemo(
    () =>
      gridItems.filter((item): item is Required<GridItem> =>
  ...
  useEffect(() => {
    const loadGridItems = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to load grid items")
        );
        setIsLoading(false);
      }
    };
  ...
  const renderGridItems = useCallback(() => {
    return visibleItems.map((item) => (
      <BentoGridItem
        ...
        className={`${
          item.className || ""
        } transition-all duration-300 ease-in-out hover:scale-[1.02] flex flex-col justify-between h-full`}
  ...
  return (
    ...
    >
      <section
        id="about"
        className="w-full px-4 sm:px-6 md:px-8 lg:px-16 xl:px-20 py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32"
        aria-label="About section with grid layout"
      >
        <BentoGrid>{renderGridItems()}</BentoGrid>
