...
export const BentoGrid: React.FC<BentoGridProps> = React.memo(
  ({ className, children, columns = 4, gap = "4" }) => {
...
export const BentoGridItem: React.FC<BentoGridItemProps> = React.memo(
  ({
    ...
    const sizeClasses = useMemo(
      () => ({
        small:
          "col-span-1 row-span-1 min-h-[200px] sm:min-h-[240px] md:min-h-[280px] lg:min-h-[320px] xl:min-h-[360px]",
        medium:
          "col-span-1 row-span-2 min-h-[300px] sm:min-h-[360px] md:min-h-[420px] lg:min-h-[480px] xl:min-h-[540px]",
        large:
          "col-span-2 row-span-2 min-h-[400px] sm:min-h-[480px] md:min-h-[560px] lg:min-h-[640px] xl:min-h-[720px]",
    ...
    const handleError = useCallback(
      (type: "lottie" | "globe" | "image", error: Error | boolean) => {
        console.error(
      ...
      navigator.clipboard
        ...
        .catch((err) => {
          ...
          notification.textContent =
            "Failed to copy email. Please try again or copy manually.";
          notification.setAttribute("role", "alert");
          notification.className =
            "fixed top-4 right-4 p-4 bg-red-500 text-white rounded shadow-lg z-50";
    ...
    const renderTechList = useCallback(
      (list: string[]) => (
        <ul
    ...
    const renderGlobe = useCallback(
      () => (
        <ErrorBoundary
          fallback={
            <div
              className="text-red-500 p-4 bg-red-100 rounded-lg"
              role="alert"
            >
        ...
        >
          <Suspense
            fallback={
              <LoadingSpinner message="Loading Globe..." size="large" />
            }
          >
            {errors.globe ? (
              <div
    ...
    const renderLottieAnimation = useCallback(
      () => (
        ...
        >
          ...
          >
            {errors.lottie ? (
              <div className="text-yellow-500" role="alert">
    ...
    const titleContainerClassNames = useMemo(
      () =>
        cn(
          titleClassName,
        ...
        <div className={contentClassNames}>
          ...
          {id === 6 && (
            <BackgroundGradientAnimation>
              <div
                className="absolute z-50 inset-0 flex items-center justify-center text-white font-bold px-3 sm:px-4 md:px-5 pointer-events-none text-sm sm:text-base md:text-lg lg:text-xl text-center"
                aria-hidden="true"
              ></div>
            </BackgroundGradientAnimation>
          )}
          ...
          <div className={titleContainerClassNames}>
            ...
            <h3
              id={`bento-item-${id}-title`}
              className="font-sans text-base sm:text-lg lg:text-3xl max-w-full lg:max-w-96 font-bold z-10 mt-2"
            >
              {title}
            </h3>
