...
function fixGeometryNaNs(geometry: THREE.BufferGeometry): {
  nanCount: number;
  fixedPositions: { index: number; original: number; fixed: number }[];
} {
  ...
  const fixValue = (index: number): number => {
    ...
    const validNeighbors = neighborIndices
      .map((i) => positions[i])
    ...
    console.warn(
      `No valid neighbors found for index ${index}. Using default value.`
    );
    return 0;
  };
  ...
  for (let i = 0; i < positions.length; i++) {
    if (!Number.isFinite(positions[i]) || Number.isNaN(positions[i])) {
      ...
      console.debug(
        `Fixed NaN/Infinite value at index ${i}: ${originalValue} -> ${fixedValue}`
      );
    }
  }
  ...
  if (nanCount > 0) {
    console.warn(
      `Fixed ${nanCount} invalid values in geometry`,
      fixedPositions
    );
    ...
    try {
      ...
      console.debug(
        "Successfully computed bounding sphere:",
        JSON.stringify(geometry.boundingSphere)
      );
    } catch (error) {
...
function calculateFixedValue(
  ...
  if (
    Number.isFinite(positions[prevIndex]) &&
    Number.isFinite(positions[nextIndex])
  ) {
...
function handleBoundingError(geometry: THREE.BufferGeometry): void {
  console.warn(
    "Invalid bounding sphere detected. Attempting to calculate fallback."
  );

  if (!geometry.attributes.position) {
    console.error(
      "Geometry has no position attribute. Cannot calculate bounding sphere."
    );
  ...
  if (validVertices === 0) {
    ...
    if (isFinite(radius) && radius > 0) {
      ...
      console.log(
        "Computed fallback bounding sphere:",
        JSON.stringify({ center: center.toArray(), radius, validVertices })
      );
    } else {
      console.error(
        "Computed radius is invalid. Setting default bounding sphere."
      );
      geometry.boundingSphere = new THREE.Sphere(new THREE.Vector3(0, 0, 0), 1);
    }
  }
...
function logFinalBoundingSphere(geometry: THREE.BufferGeometry): void {
  if (geometry.boundingSphere) {
    ...
    console.error(
      "Bounding sphere is still undefined after attempts to set it"
    );
  }
}
...
function isValidRGB(rgb: RGB): boolean {
  return ["r", "g", "b"].every((key) =>
    isValidCoordinate(rgb[key as keyof RGB], 0, 255)
  );
}
...
const validateAndFixData = (
  inputData: Position[]
): { fixedData: Position[]; invalidIndices: number[] } => {
  const invalidIndices: number[] = [];
  const fixedData = inputData.map((arc, index) => {
    const fixArcCoordinate = (
      value: number,
      min: number,
      max: number,
      name: string
      ...
      if (fixedValue !== value) {
        console.warn(
          `${name} at index ${index} fixed. Original: ${value}, Fixed: ${fixedValue}`
        );
    ...
    if (
      fixedArc.startLat === fixedArc.endLat &&
      fixedArc.startLng === fixedArc.endLng
    ) {
      console.warn(
        `Arc at index ${index} has identical start and end points. Adjusting end point.`
      );
      fixedArc.endLat = fixArcCoordinate(
        fixedArc.endLat + 1,
        -90,
        90,
        "endLat"
      );
      fixedArc.endLng = fixArcCoordinate(
        fixedArc.endLng + 1,
        -180,
        180,
        "endLng"
    ...
    if (!rgbColor) {
      console.warn(
        `Invalid color at index ${index}. Original: ${arc.color}, Fixed: #FFFFFF`
      );
...
const handleGlobeInitializationError = (
  ...
  if (retryCount.current < maxRetries) {
    retryCount.current++;
    console.log(
      `Retrying globe initialization (Attempt ${retryCount.current} of ${maxRetries})`
    );
...
export const Globe: React.FC<WorldProps> = React.memo(
  ({ globeConfig, data }) => {
    ...
    const mergedProps = useMemo(
      () => ({ ...defaultProps, ...globeConfig }),
      [globeConfig]
    );
    const hexPolygonColor = useMemo(
      () => mergedProps.polygonColor || defaultProps.polygonColor,
    ...
    const handleError = useCallback(
      (error: unknown) => {
        ...
        const errorMessage = `Failed to initialize globe: ${
          error instanceof Error ? error.message : "Unknown error"
        }`;
        ...
        if (data.length === 0) {
          setError(
            "No data available for globe initialization. Please check your data source."
          );
          return;
        }
        ...
        if (retryCount.current < MAX_RETRIES.current) {
          retryCount.current++;
          console.log(
            `Retrying globe initialization (Attempt ${retryCount.current} of ${MAX_RETRIES.current})`
          );
          setTimeout(initializeGlobe, 1000 * Math.pow(2, retryCount.current)); // Exponential backoff
        } else {
          console.error(
            `Failed to initialize globe after ${MAX_RETRIES.current} attempts. Please try refreshing the page.`
          );
    ...
    const initializeGlobe = useCallback(() => {
      ...
      if (data.length === 0) {
        setError(
          "Globe data is not available. Please try again later or check your data source."
        );
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const globeObj = globe as unknown as {
          __globeObjRef?: { geometry?: THREE.BufferGeometry };
        };
        if (globeObj.__globeObjRef?.geometry instanceof THREE.BufferGeometry) {
          ...
          if (result.nanCount > 0) {
            console.warn(
              `Fixed ${result.nanCount} NaN/Infinite values in globe geometry`,
          ...
          if (!isValidGeometry(geometry)) {
            throw new Error(
              "Failed to create a valid geometry after all fixes. Please try refreshing the page."
            );
          }
          ...
          throw new Error(
            "Globe geometry is not available or not a BufferGeometry. Please try refreshing the page."
          );
        }
...
const validatePositiveNumber = (
  ...
  if (typeof value !== "number" || !isFinite(value) || value <= 0) {
    console.warn(
      `Invalid ${propertyName}: ${value}. Using default value: ${defaultValue}`
    );
    return defaultValue;
  }
  return value;
...
const calculateRingRepeatPeriod = (): number => {
  ...
  if (
    typeof arcTime === "number" &&
    typeof arcLength === "number" &&
    typeof rings === "number" &&
    rings !== 0
  ) {
...
const World: React.FC<WorldProps> = React.memo(({ globeConfig, data }) => {
  ...
  const handleContextLoss = useCallback((event: WebGLContextEvent) => {
    ...
    setError(
      new Error(
        "WebGL context lost. Please wait while we attempt to restore it."
      )
    );
  }, []);
