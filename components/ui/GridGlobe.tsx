  ...
  () =>
    import("./Globe")
      ...
      .catch((err) => {
        ...
        return () => (
          <ErrorMessage message="Failed to load Globe component. Please try refreshing the page." />
        );
      }),
  {
    ssr: false,
      ...
      <ErrorBoundary
        fallbackRender={({ error, resetErrorBoundary }) => (
          <div className="text-center">
            <ErrorMessage
              message={`Failed to load Globe component: ${error.message}`}
            />
            <button
...
const useArcGeneration = () => {
  ...
  const createArc = useCallback(
    (
      ...
      if (arc.startLat === arc.endLat && arc.startLng === arc.endLng) {
        ...
        arc.endLng = validateCoordinate(
          arc.endLng + MIN_ARC_DISTANCE,
          -180,
          180
        );
      }
  ...
  const generateSampleArcs = useCallback(
    async (count: number): Promise<Position[]> => {
      if (!workerRef.current) {
        try {
          ...
          throw new Error(
            `Failed to initialize arc generator: ${
      ...
      return new Promise<Position[]>((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          cleanupWorker();
          reject(
            new Error(`Arc generation timed out after ${GENERATION_TIMEOUT}ms`)
          );
        }, GENERATION_TIMEOUT);
        ...
        const handleMessage = (event: MessageEvent) => {
          ...
          if (Array.isArray(event.data) && event.data.every(isValidPosition)) {
            ...
            const errorMessage = Array.isArray(event.data)
              ? `Invalid data received from worker: ${
                  event.data.length
                } items, ${event.data.filter(isValidPosition).length} valid`
...
const GridGlobe: React.FC = React.memo(() => {
  ...
  const renderError = useCallback(
    (errorMessage: string, retryHandler: () => void) => (
  ...
  const WorldMemo = useMemo(
    () => <GlobeWrapper globeConfig={memoizedGlobeConfig} data={sampleArcs} />,
  ...
  const content = useMemo(() => {
    if (error) {
      ...
      return renderError(
        `Failed to render Globe: ${error.message}. Please check your network connection and try again.`,
        handleRetryWithCount
      );
    }
    ...
    if (isLoading) {
      return (
        <LoadingSpinner
          message="Generating globe data... This may take a few moments."
          aria-live="polite"
          size="large"
        />
      );
    ...
    if (webGLContextLost) {
      return (
        <ErrorMessage message="WebGL context lost. Please refresh the page." />
      );
    }
    ...
    return (
      ...
      >
        <Suspense
          fallback={
            <LoadingSpinner
              message="Rendering globe... Please wait."
              aria-live="polite"
              size="large"
            />
          }
  ...
  useEffect(() => {
    ...
    if (canvas) {
      canvas.addEventListener(
        "webglcontextlost",
        handleWebGLContextLost as EventListener
      );
      canvas.addEventListener(
        "webglcontextrestored",
        handleWebGLContextRestored as EventListener
      );
    }
  ...
  return (
    <div
      ref={containerRef}
      className="grid-globe-container"
      aria-busy={isLoading}
      aria-live="polite"
    >
      {content}
